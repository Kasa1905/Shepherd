...
def handler(event, context):
    ...
    try:
        ...
        # Send notification
        send_notification(
            sns_client, sns_topic_arn, verification_result, project_name, environment
        )
...
def verify_successful_backup(backup_client, rds_client, backup_job_id, resource_arn):
    ...
    try:
                ...
                backup_details.update(
                    {
                        ...
                        "latest_restorable_time": cluster[
        ...
        backup_details["verification_timestamp"] = datetime.now(
            timezone.utc
        ).isoformat()
...
def handle_failed_backup(backup_client, backup_job_id, resource_arn):
    ...
    try:
        ...
        logger.error(
            f"Backup failed: {backup_job_id} - {failure_details['status_message']}"
        )
