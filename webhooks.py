...
# Exposed module-level defaults for tests to patch
...
def dispatch_webhook(
    event: WebhookEvent, webhook_config: WebhookConfig
) -> Dict[str, Any]:
    """
    ...
    if webhook_config.secret:
        headers["X-Shepherd-Signature"] = generate_signature(
    ...
    try:
        ...
        return {"status_code": None, "response_text": str(e), "success": False}
...
def dispatch_webhook_async(
    event: WebhookEvent, webhook_config: WebhookConfig, webhook_manager=None
):
    """
    ...
    def _dispatch():
        ...
        if webhook_manager:
            webhook_manager.stats.setdefault(
                webhook_config.url, {"success": 0, "failure": 0}
            )
            if result["success"]:
...
def dispatch_with_retry(
    ...
    if not result["success"] and (
        ...
        raise requests.ConnectionError(
            f"Webhook delivery failed: {result['response_text']}"
        )
    return result
...
class WebhookManager:
    ...
    def _load_webhooks(self) -> List[WebhookConfig]:
        # Use module-level defaults (which tests can patch) with env fallbacks
        urls_raw = (
            WEBHOOK_URLS if isinstance(WEBHOOK_URLS, str) else ",".join(WEBHOOK_URLS)
        )
        urls = [u.strip() for u in urls_raw.split(",") if u.strip()]
        secret = WEBHOOK_SECRET
        events = (
            WEBHOOK_EVENTS.split(",")
...
# Optional global webhook manager instance (set by application when desired)
...
def trigger_webhook_on_create(config: Dict[str, Any], webhook_manager_instance=None):
    ...
    elif webhook_manager is not None:
...
def trigger_webhook_on_rollback(
    config: Dict[str, Any], target_version: int, webhook_manager_instance=None
):
    event = WebhookEvent(
