...
# Backwards-compatibility shims for tests expecting legacy metric names
# These will be assigned after metric objects are created below.
...
# Legacy alias for tests
DATABASE_OPERATION_COUNT = mongodb_operations_total
...
# Legacy alias for tests
DATABASE_OPERATION_DURATION = mongodb_operation_duration_seconds
...
active_sessions = Gauge(
    "active_sessions", "Number of active user sessions", registry=metrics_registry
)
            ...
            try:
                if hasattr(func, "__name__"):
                    if "create" in func.__name__:
                        config_operations_total.labels(
                            operation="create", status=status
                        ).inc()
...
def track_auth_event(
    method: str, success: bool, username: Optional[str] = None
) -> None:
    """
