...
class DatabaseConnection:
    ...
    def _initialize_connection(self):
        ...
        try:
            ...
            if is_replica_set:
                ...
                logger.info("Detected replica set configuration, using enhanced connection options")
    ...
    def _is_replica_set_uri(self, uri: str) -> bool:
        ...
        return ('replicaSet=' in uri or 
                ',' in uri.split('://')[1].split('/')[0] or  # Multiple hosts
                'readPreference=' in uri)
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            configurations_collection.create_index([("name", 1)], unique=True)
            configurations_collection.create_index([("environment", 1)])
            configurations_collection.create_index([("tags", 1)])
            configurations_collection.create_index([("owner", 1)])
            configurations_collection.create_index([("created_at", -1)])
            configurations_collection.create_index([("updated_at", -1)])
            
            # Compound indexes for common queries
            configurations_collection.create_index([("environment", 1), ("created_at", -1)])
            configurations_collection.create_index([("tags", 1), ("environment", 1)])
            ...
            if self._is_replica_set_uri(mongodb_uri):
                ...
                if rs_status:
                    ...
                    if rs_status['primary'] is None:
                        health_status['errors'].append('No primary node found in replica set')
    ...
    def check_replica_set_lag(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'max_lag_seconds': max([s['lag_seconds'] for s in secondaries_lag]) if secondaries_lag else 0
            }
    ...
    def test_failover_readiness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if health['replica_set'] is None:
                ...
                readiness['recommendations'].append('Configure replica set for high availability')
                return readiness
            ...
            if lag_info and lag_info['max_lag_seconds'] > 60:  # More than 1 minute lag
                readiness['issues'].append(f"High replication lag: {lag_info['max_lag_seconds']} seconds")
                readiness['recommendations'].append('Investigate network or performance issues')
...
class ConfigurationManager:
    ...
    def create_config(self, config_data: Dict[str, Any], updated_by: str = None, change_notes: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing_config = self.collection.find_one({"config_id": config_data["config_id"]})
            if existing_config:
                raise ValueError(f"Configuration with config_id '{config_data['config_id']}' already exists")
            ...
            logger.info(f"Created configuration: {config_data['config_id']} (version 1)")
            return document
    ...
    def get_latest_config(self, config_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if document:
                ...
                logger.info(f"Retrieved latest configuration: {config_id} (version {document['version']})")
    ...
    @track_db_operation('insert', 'configurations')
    def update_config(self, config_id: str, new_settings: Dict[str, Any], updated_by: str = None, change_notes: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not latest_config:
                raise ValueError(f"Configuration with config_id '{config_id}' not found")
            ...
            new_document = {
                ...
                "created_at": latest_config["created_at"],  # Preserve original creation time
                "updated_at": current_time,
                "updated_by": updated_by,
    ...
    @track_db_operation('insert', 'configurations')
    def rollback_config(self, config_id: str, target_version: int, updated_by: str, change_notes: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            target_config = self.collection.find_one({"config_id": config_id, "version": target_version})
            if not target_config:
                raise ValueError(f"Version {target_version} not found for config_id '{config_id}'")
            ...
            new_document = {
                ...
                "settings": target_config["settings"],  # Copy settings from target version
                "created_at": target_config["created_at"],  # Preserve original creation time
                "updated_at": current_time,
                "updated_by": updated_by,
                "change_notes": change_notes or f"Rolled back to version {target_version}"
            }
            ...
            logger.info(f"Rolled back {config_id} to version {target_version}, created version {new_version}")
            return new_document
    ...
    @track_db_operation('find', 'configurations')
    def get_config_version(self, config_id: str, version: int) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            document = self.collection.find_one({"config_id": config_id, "version": version})
            
            if document:
                # Convert ObjectId to string for JSON serialization
                document["_id"] = str(document["_id"])
                logger.info(f"Retrieved configuration version: {config_id} (version {version})")
    ...
    def get_config_history(self, config_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(documents)} versions for configuration: {config_id}")
            return documents
    ...
    def query_configs(self, app_name: Optional[str] = None, 
        ...
        try:
            ...
            logger.info(f"Retrieved {len(documents)} latest configurations (app_name={app_name}, environment={environment})")
            return documents
    ...
    def delete_config(self, config_id: str) -> int:
        ...
        try:
            ...
            logger.info(f"Deleted {deleted_count} versions of configuration: {config_id}")
