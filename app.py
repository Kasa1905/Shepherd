...
from webhooks import (
    WebhookManager,
    WebhookEvent,
    trigger_webhook_on_create,
    trigger_webhook_on_update,
    trigger_webhook_on_rollback,
)
...
def create_config_ui():
    ...
    try:
        ...
        config_manager.create_config(
            config_data, updated_by=updated_by, change_notes=change_notes
        )
...
def view_config(config_id):
    ...
    try:
        ...
        return render_template(
            "details.html", config=config, settings_json=settings_json
        )
...
def update_config(config_id):
    ...
    try:
        ...
        logger.info(
            f"Updated configuration: {config_id} (new version: {updated_config['version']})"
        )
        return redirect(url_for("view_config", config_id=config_id))
...
def config_history(config_id):
    ...
    try:
        ...
        logger.info(
            f"Viewed configuration history: {config_id} ({len(versions)} versions)"
        )
...
def compare_versions(config_id, version1, version2):
    ...
    try:
        ...
        return render_template(
            "compare.html", config_id=config_id, version1=config_v1, version2=config_v2
        )
...
def rollback_config(config_id, target_version):
    ...
    try:
        ...
        updated_by = (
            g.user.get("username")
            if isinstance(g.user, dict) and "username" in g.user
            else "test-user"
        )
        # Perform rollback
        new_config = config_manager.rollback_config(
            config_id, target_version, updated_by=updated_by, change_notes=change_notes
        )

        flash(
            f'Successfully rolled back to version {target_version}. Created new version {new_config["version"]}.',
            "success",
        )
        logger.info(
            f"User {updated_by} rolled back {config_id} to version {target_version}"
        )
        return redirect(url_for("view_config", config_id=config_id))
...
def create_config():
    ...
    try:
        ...
        result = config_manager.create_config(
            data, updated_by=updated_by, change_notes=change_notes
        )
...
def update_configuration(config_id):
    ...
    try:
        ...
        logger.info(
            f"Updated configuration: {config_id} (new version: {result['version']})"
        )
        return jsonify(result), 200
...
def get_configuration_history(config_id):
    ...
    try:
        ...
        logger.info(
            f"Retrieved configuration history: {config_id} ({len(result)} versions)"
        )
        return jsonify(result), 200
...
def query_configurations():
    ...
    try:
        ...
        logger.info(
            f"Queried configurations: app_name={app_name}, environment={environment} ({len(result)} results)"
        )
        return jsonify(result), 200
...
def get_config_version(config_id, version):
    ...
    try:
        ...
        if not result:
            return (
                jsonify({"error": "Configuration version not found", "status": 404}),
                404,
            )
...
def rollback_config_api(config_id, target_version):
    ...
    try:
        ...
        change_notes = data.get(
            "change_notes", f"Rolled back to version {target_version} via API"
        )

        # Perform rollback
        result = config_manager.rollback_config(
            config_id, target_version, updated_by, change_notes
        )
        ...
        logger.info(
            f"Rolled back configuration via API: {config_id} to version {target_version} by {updated_by}"
        )
        return jsonify(result), 200
...
def health_check():
    ...
    try:
        ...
        return (
            jsonify({"status": "healthy", "database": "connected", "stats": stats}),
            200,
        )
        ...
        try:
            if request.is_json:
                try:
                    ...
                    return (
                        jsonify({"error": "Invalid JSON payload", "status": 400}),
                        400,
                    )
                if any(k in data for k in ["name", "data"]):
                    ...
                    updated_by = data.get("updated_by") or (
                        (g.user or {}).get("username") if hasattr(g, "user") else None
                    )
                    change_notes = data.get("change_notes", "")
                    result = config_manager.create_config(
                        transformed, updated_by=updated_by, change_notes=change_notes
                    )
                    trigger_webhook_on_create(result, webhook_manager)
                    update_config_metrics()
                    return jsonify(result), 201
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port, debug=debug)  # Allow external connections
